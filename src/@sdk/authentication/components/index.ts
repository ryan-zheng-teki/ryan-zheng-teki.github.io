...
interface SocailLoginProps {
  ...
  onLogoutFailure: () => {}
  onLogoutSuccess: () => {}
  provider: keyof SocailLoginConfig.provider;
  redirect:
 ...
 * @constructor
 */
const SocialLogin = (WrappedComponent: Component) =>
            ...
            () => {
              if (autoLogin || this.accessToken) {
                if (this.fetchProvider && !this.accessToken) {
                  this.sdk.login(appId, redirect).catch(this.onLoginFailure);
                } else {
                  this.sdk
                    .checkLogin(true)
