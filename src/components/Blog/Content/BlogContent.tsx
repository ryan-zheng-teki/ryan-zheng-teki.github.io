      ...
      <CurrentCategoryContext.Consumer>
        {({ currentCategory }) => {
          if (currentCategory === undefined || currentCategory === null) {
            ...
            return (
              <p>
                The author is too lazy, he does not have any blogs currrently
              </p>
            );
          }
          return (
            <BlogsForCategoryQuery variables={{ categoryId: currentCategory }}>
              {({ data }) => {
                if (
                  data &&
                  data.blogsForCategory &&
                  data.blogsForCategory.length > 0
                ) {
                  const blogs =
                    data &&
                    data.blogsForCategory.map((blog) => {
                ...
                return (
                  <p>
                    The author has not created any blogs for {currentCategory}
                  </p>
                );
              }}
